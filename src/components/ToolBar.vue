    ...
    this.tools.forEach((item) => {
        if(item.key != null) {
            if(item.type === 'inputRange') {
                this.$set(this.valueObj, item.key[0], item.defaultValue[0] || undefined)
                this.$set(this.valueObj, item.key[1], item.defaultValue[1] || undefined)
            } else {
                this.$set(this.valueObj, item.key, item.defaultValue || undefined) // 需要用$set方法去设置data中valueObj的属性值，不然无法实现双向绑定
            }
        }
    })
